...
<?php if ($comments) : ?>
	...
	<?php $cc = 1; foreach ($comments as $comment) : ?>

		<li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
			<h3 class="commenttitle"><?php comment_author_link() ?></h3>
			<p class="commentmeta">
...
<?php if ('open' == $post-> comment_status) : ?>
	...
	<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
		...
		<div id="commentform"><p>You must be <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>">logged in</a> to post a comment.</p></div>
	...
	<?php else : ?>
		...
		<p>Logged in as <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url() ?>" title="<?php _e('Log out of this account') ?>">Logout &raquo;</a></p>

		<?php else : ?>
	
		<p>
		<input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="30" tabindex="1" />
		<label for="author">Name <span class="note"><?php if ($req) _e('(required)'); ?></span></label>
		</p>
		
		<p>
		<input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="30" tabindex="2" />
		<label for="email">E-mail <span class="note">(<?php if ($req) _e('required, '); ?>never displayed)</span></label>
		</p>
